...
select 
    *
from tblStudent s inner join tblSemester m on s.seq = m.student_seq;


select
    *
from tblSemester s inner join tblResult r on s.student_seq = r.student_seq and s.subject_seq = r.subject_seq;
...
select
    ...
    sb.name as "과목명",
    r.score as "점수"
from tblSemester s inner join tblResult r on s.student_seq = r.student_seq and s.subject_seq = r.subject_seq inner join tblStudent st on st.seq = s.student_seq inner join tblSubject sb on sb.seq = s.subject_seq;
