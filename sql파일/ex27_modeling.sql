...
데이터베이스 구축
    ...
    1. 데이터베이스 모델링
        ...
        - 요구분석 > 정보(Raw) 도출 + 수집 > 분석(구분, 공통) > 저장 구조(릴레이션, 속성...)
            > 도식화(설계도) > ERD
        - 데이터베이스 모델링의 결과물 : ERD (최종 산출물)
        - 아직 이 단계에서는 DBMS의 종류를 결정하지 않는다.
            > 추상적
...
1. ERD, Entity Relationship Diagram
    ...
    - 손, 오피스, 전문틀 (eXERD, ER-WIN, SQL Developer Modeler)
    
2. Entity, 엔티티
    - 다른 Entity와 분류(구분)될 수 있고, 다른 Entity에 대해 정해진 관계를 맺을 수 있는 데이터 단위
    - 릴레이션(속성,속성,속성...) -> 테이블(컬럼,컬럼,컬럼)
    - 릴레이션 = 레코드 = 객체 = 엔티티
    - (릴레이션 == 엔티티) = 테이블
...
1. 1+2 > 팀회의 없이 각각 > 팀원별 각각 따로 구현
    - 각자 본인이 모든 요구분석서 해석 > 데이터 도출 > 엔티티(속성) 도출 > 엔티티 그리기
    
2. 1번의 산출물 팀회의
    - 엔티티가 올바르게 도출 되었는지
    - (엔티티 안의) 속성이 올바르게 도출되었는지
    - 누락된 요구사항 확인
    - 한명씩 돌아가면서 자기 ERD 설명
    - 모든 ERD > 통합 > 최종 엔티티 결정

3. 3+4 > 팀회의 없이 각각 > 팀원별 각각 따로 구현
...
모델링 작업 > ERD > 정규화(Normalization) > 안정성 높고, 작업하기 편한 ERD


정규화, Normalization
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며, 데이터 종속성을 최소화하기 위해 자료구조(ERD)를 수정하는 작업
...
관계형 데이터베이스 시스템이 지향하는 데이터베이스의 상태
    ...
    2. 중복값을 저장하지 않는다. 
        > 동일한 성격의 데이터를 2개 이상의 테이블에 저장하지 않는다.
    3. 하나의 셀
        > 원자값 (단일값)
...
2. 갱신 이상, Update Anormaly
    - 동일한 데이터가 2개 이상의 테이블에 존재할 때 > 그 중 1개는 수정했지만, 다른 1개를 수정하지 못했을 때
        > 두 데이터 간의 이상 오류가 발생한다. > 즉, 2개의 데이터 중 어떤 데이터가 올바른 데이터인지 구분을 하지 못하는 이상 발생
        
3. 삭제 이상, Deletion Anormaly
    - 특정 ㅌ테이블에서 데이터를 삭제할 때 원하지 않는 데이터까지 같이 지워지는 상황
...
제 3정규화, 3NF
    ...
    ex) 정규화.xlsx
        - 등록금 및 학과사무실은 학과에 종속됨. 기본키인 학번이 아니라.
...
역정규화
	...
	- 2개 이상의 테이블 > 1개 테이블
    - 수업 중엔 사용 금지... 이걸 판단할 현실적인, 실무의 데이터가 없다.
    - 1~3 정규화 잘하자. 이것만으로 거의 97~8% DB관리 잘한 거라고 볼 수 있다.
    
    ex) 정규화.xlsx
        - 제시된 상황에서는 join할 때 느려지므로, 1개 테이블의 형태로 다시 돌아감
